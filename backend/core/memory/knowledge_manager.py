...
class KnowledgeManager:
    ...
    async def add_knowledge(
        ...
        knowledge = Knowledge(
            content=content, knowledge_type=knowledge_type, category=category, locked=locked, tags=tags or []
        )
    ...
    async def retrieve_context(
        self, query: str, top_k: int = 10, categories: Optional[List[KnowledgeCategory]] = None
    ) -> List[str]:
        """
    ...
    async def get_by_category(
        self, category: KnowledgeCategory, knowledge_type: Optional[KnowledgeType] = None
    ) -> List[Knowledge]:
        """按分类获取知识"""
