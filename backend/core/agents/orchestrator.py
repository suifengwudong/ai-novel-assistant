...
class NovelAssistantOrchestrator:
    ...
    def _build_workflow(self):
        ...
        # 条件分支
        workflow.add_conditional_edges("validate", self._should_refine, {"refine": "refine", "finish": END})
...
"""
    ...
    async def _retrieve_context(self, state: AgentState) -> AgentState:
        ...
        # 检索核心知识
        core_knowledge = await self.km.retrieve_context(query=state["user_input"], top_k=10)
        ...
        if state["task_type"] in ["generate", "continue"]:
            ...
            try:
                ...
                state["summaries"] = {f"chapter_{i}": s.content for i, s in enumerate(recent_summaries)}
            except:
                state["summaries"] = {}
...
"""
    ...
    async def _validate_output(self, state: AgentState) -> AgentState:
        ...
        if not state.get("output"):
            state["validation_result"] = {"passed": False, "issues": ["生成内容为空"]}
            return state
        ...
        try:
            ...
            state["messages"].append(f"校验结果: {'通过' if validation_result['passed'] else '未通过'}")

            logger.info(f"校验结果: {validation_result['passed']}")
        except Exception as e:
            logger.error(f"校验失败: {e}")
            state["validation_result"] = {"passed": True, "issues": []}  # 降级：校验失败则默认通过
