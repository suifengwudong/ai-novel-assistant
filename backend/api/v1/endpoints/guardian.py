...
async def check_deviation(request: CheckDeviationRequest, guardian=Depends(get_outline_guardian)):
    """
    检查正文与大纲的偏离度
    """
    try:
        # 构造临时节点对象用于检查
        from core.structure.models import NodeType, PlotNode

        # 临时创建一个节点对象
        temp_node = PlotNode(
            id="temp", title=request.outline_title, description=request.outline_desc, type=NodeType.CHAPTER
        )
...
async def validate_logic(request: LogicValidationRequest, validator=Depends(get_logic_validator)):
    """
    验证内容逻辑一致性
    """
    try:
