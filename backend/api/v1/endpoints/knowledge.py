...
@router.post("/add")
async def add_knowledge(item: KnowledgeItem, vector_store=Depends(get_vector_store)):
    """添加知识条目"""
    try:
        # 生成唯一ID
        import uuid
        ...
        await vector_store.add_texts(
            texts=[item.content], metadatas=[{"category": item.category, **(item.metadata or {})}], ids=[doc_id]
        )
...
@router.get("/retrieve")
async def retrieve_knowledge(query: str, top_k: int = 5, category: Optional[str] = None):
    """检索知识"""
    try:
        # 暂时返回mock数据
        return {"results": [{"content": f"关于 {query} 的相关背景知识...", "score": 0.92}]}
    except Exception as e:
...
async def add_relation(
    source: str, target: str, relation: str, description: str = "", knowledge_graph=Depends(get_knowledge_graph)
):
    """添加实体关系"""
    try:
...
async def get_entity_relations(
    entity: str, relation_type: Optional[str] = None, knowledge_graph=Depends(get_knowledge_graph)
):
    """获取实体的所有关系"""
    try:
